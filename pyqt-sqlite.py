...
__appname__ = 'PyQt SqLite Браузер базы данных'
...
class MainWindow(QtWidgets.QMainWindow, uic.loadUiType('mainwindow.ui')[0]):
    ...
    def on_actionAbout_triggered(self):
        QtWidgets.QMessageBox.about(self, "About this software",
                                    "This software is written for teaching purposes. Don't use in production!!! My purpose is to demonstrate how PyQt widgets can be used to develop software.\n\n Main developer: Mustafa Baser <mbaser@mail.com>")
    ...
    def update_tables_table(self):
        ...
        for typ in ('table', 'view'):
            ...
            for tb in tables:
                if typ == 'table':
                    c_icon = "sc_inserttable.png"
                else:
                    c_icon = "sc_dbviewtablenames.png"
                ...
                q = self.current_database.exec(
                    "SELECT sql FROM sqlite_master WHERE tbl_name = '%s' AND type = '%s'" % (tb, typ))
                tb_schema_str = ''
                if q.next():
    ...
    def show_warning(self, text):
        QtWidgets.QMessageBox.warning(self, "Info",
                                      "Could not execute query. Error message from database engine is:\n" + text)
    ...
    def update_recent_files(self):
        ...
        for i, rc in enumerate(self.recent_files):
            recent_file_action = QtWidgets.QAction('&%d %s' % (i + 1, rc), self,
                                                   triggered=partial(self.loadDatabase, rc))
